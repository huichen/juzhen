...
public:
  ...
  vector(const matrix<DataType> &m) {
    matrix<DataType>::_Data = DataPtr<DataType>::Type(new DataArray<DataType>(m.getDataPtr(), m.nRow*m.nCol)); 
  ...
  size_t size() const {return matrix<DataType>::nCol==1?matrix<DataType>::nRow:matrix<DataType>::nCol;}

  template<typename T> const vector<DataType> operator+(const vector<T>& rhs) const {
    return matrix<DataType>::operator+(rhs);
  }

  template<typename T> vector<DataType>& operator+=(const vector<T>& rhs) {
    return matrix<DataType>::operator+=(rhs);
  }
  ...
  template<typename T> const DataType operator* (const vector<T> &rv) {
    assert(size() == rv.size());
    DataType res = 0;
  ...
  template<typename T> const vector<DataType> operator/(const T rhs) const {
    return matrix<DataType>::operator/(rhs);
  }

  template<typename T> vector<DataType>& operator/=(const T rhs) {
    return matrix<DataType>::operator/=(rhs);
  }

  vector<DataType> &trans() {return *this;};
  vector<DataType> &herm() {return conj();};
...
template<typename DataType> vector<DataType> trans(const vector<DataType> &v) {
  return v;
}

template<typename DataType> vector<DataType> herm(const vector<DataType> &v) {
  vector<DataType> v1 = v;
  v1.herm();
  return v1;
}

template<typename DataType> vector<DataType> conj(const vector<DataType> &v) {
  vector<DataType> v1 = v;
  v1.conj();
  return v1;
}
...
template<typename DataType> const vector<DataType> operator*(const double lhs, const vector<DataType> &ma) {
...
template<typename DataType> const vector<DataType> operator*(const CD lhs, const vector<DataType> &ma) {
  vector<DataType> m(ma.size());
  for (size_t i=0; i<ma.nRow; i++)
      m(i) = ma(i)*lhs; 
  return m;
...
template<typename DataType> const vector<DataType> operator*(const matrix<DataType>& ma, const vector<DataType>& v) {
  matrix<DataType> m = v;
  vector<DataType> v1 = ma*m;
  return v1;
}

template<typename DataType> vector<DataType>& operator*=(matrix<DataType>& ma, const vector<DataType>& v) {
  matrix<DataType> m = v;
  ma *= m;
  return ma;
}
